...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { reuniao_id, titulo, data_inicio, duracao_minutos, convidados } = await req.json();
    if (!reuniao_id || !data_inicio || !duracao_minutos) {
    ...
    const { data: reuniao, error: rErr } = await sb
      ...
      .select("id, google_event_id, calendar_id, lead_id")
      .eq("id", reuniao_id)
      .maybeSingle();
    if (rErr || !reuniao) {
    ...
    if (!eventId) throw new Error("Evento Google não vinculado a esta reunião.");

    const attendees = (convidados || []).filter((e: string) => !!e).map((email: string) => ({ email }));
    ...
    if (!gResp.ok) {
      return new Response(JSON.stringify({ error: gJson?.error?.message || "Falha ao atualizar evento Google" }), {
        status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    await sb.from("reunioes_agendadas").update({
      titulo,
    ...
    return new Response(JSON.stringify({ ok: true, event: gJson }), {
